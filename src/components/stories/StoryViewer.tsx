...
export function StoryViewer({ story, onClose, currentUserId, onStoryUpdated }: StoryViewerProps) {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-sm mx-auto p-0 bg-black border-none overflow-hidden animate-scale-in">
        <div className="relative w-full h-[600px] flex flex-col">
          ...
          {/* Header */}
          <div className="absolute top-6 left-2 right-2 z-10 flex items-center justify-between animate-fade-in">
            ...
            <Button
              ...
              size="icon"
              variant="ghost"
              className="text-white hover:bg-white/20 h-6 w-6 transition-all duration-200 hover:scale-110"
            >
              <X className="h-3 w-3" />
            </Button>
          </div>
          ...
          {/* Photo Counter */}
          {totalPhotos > 1 && (
            <div className="absolute bottom-12 left-2 right-2 z-10 animate-fade-in">
          ...
          {/* Footer */}
          {isOwnStory && (
            <div className="absolute bottom-2 left-2 right-2 z-10 animate-fade-in">
